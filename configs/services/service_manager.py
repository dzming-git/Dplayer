...
def start_service(svc_key, force=False):
    ...
    log.runtime('INFO', f"启动服务: {svc['display_name']} (端口: {port})")
    
    # 检查端口
    if _port_listening(port):
        pid = _pid_for_port(port)
        log.runtime('INFO', f"服务已在运行 (PID: {pid})")
    ...
    try:
        ...
        log.runtime('INFO', f"进程已启动: PID={proc.pid}")
        
        # 等待端口就绪
        for _ in range(20):
            time.sleep(0.5)
            if _port_listening(port):
                log.runtime('INFO', f"服务启动成功 (PID: {proc.pid}, 端口: {port})")
                return {'success': True, 'message': '启动成功', 'pid': proc.pid}
        
        return {'success': True, 'message': '进程已启动，等待端口就绪', 'pid': proc.pid}
    
    except Exception as e:
        log.debug('ERROR', f"启动失败: {e}")
...
def stop_service(svc_key):
    ...
    log.runtime('INFO', f"停止服务: {svc['display_name']}")
    
    pid = _pid_for_port(port) or _get_pid_from_file(pid_file)
    
    if not pid:
        log.runtime('INFO', "服务未在运行")
    ...
    try:
        ...
        log.runtime('INFO', f"服务已停止 (PID: {pid})")
...
def install_nssm_service(svc_key, runtime_dir: str | None = None):
    ...
    if result.returncode == 0:
        ...
        log.runtime('INFO', f"NSSM服务已安装: {service_name}（工作目录: {work_dir}）")
        return {'success': True, 'message': f'服务已安装: {service_name}', 'work_dir': str(work_dir)}
    else:
        log.debug('ERROR', f"安装服务失败: {result.stderr}")
...
def uninstall_nssm_service(svc_key):
    ...
    if result.returncode == 0:
        log.runtime('INFO', f"NSSM服务已卸载: {service_name}")
