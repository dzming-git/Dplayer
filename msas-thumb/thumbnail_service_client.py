...
logger = logging.getLogger('thumbnail_client')
...
class ThumbnailServiceClient:
    ...
    def __init__(self, service_url=None, timeout=10, max_retries=3, fallback_enabled=True):
        ...
        logger.info(f"缩略图服务客户端初始化: URL={self.service_url}, fallback={fallback_enabled}")
    ...
    def _make_request(self, method, endpoint, data=None, params=None):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f"连接失败: {url}, 尝试 {attempt + 1}/{self.max_retries}")
                if attempt == self.max_retries - 1:
                    self.service_available = False
                    return None
                    
            except requests.exceptions.HTTPError as e:
                logger.error(f"HTTP错误: {url}, status={e.response.status_code}")
                return None
    ...
    def check_health(self):
        ...
        if result and result.get('status') == 'healthy':
            ...
            logger.debug(f"服务健康检查通过: {result}")
            return True
        else:
            self.service_available = False
            logger.warning(f"服务健康检查失败: {result}")
            return False
    ...
    def generate_thumbnail(self, video_path, video_hash, output_format='gif'):
        ...
        if result and result.get('success'):
            logger.info(f"成功请求缩略图生成: task_id={result.get('task_id')}, video_hash={video_hash}")
            return result
        else:
            logger.error(f"请求缩略图生成失败: video_hash={video_hash}, result={result}")
            return None
    ...
    def get_thumbnail_file(self, video_hash):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f"获取缩略图文件失败: video_hash={video_hash}, 尝试 {attempt + 1}/{self.max_retries}, 错误={str(e)}")
                if attempt == self.max_retries - 1:
                    return None
    ...
    def regenerate_thumbnail(self, video_path, video_hash, output_format='gif'):
        ...
        if result and result.get('success'):
            logger.info(f"成功请求重新生成缩略图: task_id={result.get('task_id')}, video_hash={video_hash}")
            return result
        else:
            logger.error(f"请求重新生成缩略图失败: video_hash={video_hash}, result={result}")
            return None
    ...
    def wait_for_completion(self, task_id, timeout=30, check_interval=1):
        ...
        while time.time() - start_time < timeout:
            ...
            if status is None:
                logger.warning(f"查询任务状态失败: task_id={task_id}")
                return None
            ...
            if task_status == 'completed':
                ...
                logger.error(f"任务失败: task_id={task_id}, error={status.get('error')}")
                return status
            
            time.sleep(check_interval)
        
        logger.warning(f"任务超时: task_id={task_id}, timeout={timeout}s")
        return None
