...
def setup_auth_middleware(app):
    ...
    def check_auth():
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            logger.warning(f"[Auth] 未携带 Token，拒绝访问: {path}")
            return jsonify({
                'success': False,
                'data': None,
                'message': '未授权，请先登录',
        ...
        if not payload:
            logger.warning(f"[Auth] Token 无效或已过期，拒绝访问: {path}")
            return jsonify({
                'success': False,
                'data': None,
        ...
        if payload.get('type') != 'access':
            logger.warning(f"[Auth] Token 类型错误: {payload.get('type')}，路径: {path}")
            return jsonify({
                'success': False,
                'data': None,
                'message': 'Token 类型错误',
        ...
        logger.debug(f"[Auth] 鉴权通过: user={g.username}, path={path}")
        return None  # 继续处理请求
