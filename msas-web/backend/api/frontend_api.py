...
def check_auth():
    ...
    try:
        ...
        logger.error(f"检查认证状态失败: {e}", exc_info=True)
        return jsonify({
            'authenticated': False,
            'user': None
        })
...
def upload_video():
    ...
    try:
        ...
        logger.info(f"视频上传成功: {title} (ID: {video.id})")
...
def delete_video(video_id):
    ...
    try:
        ...
        logger.info(f"删除视频成功: {video.title} (ID: {video_id})")
...
def update_video(video_id):
    ...
    try:
        ...
        db.session.commit()
        logger.info(f"更新视频成功: {video.title} (ID: {video_id})")
        ...
        logger.error(f"更新视频失败: {e}", exc_info=True)
        db.session.rollback()
        return jsonify({
            'success': False,
...
def update_tag(tag_id):
    ...
    try:
        ...
        logger.info(f"更新标签成功: {tag.name} (ID: {tag_id})")
...
def delete_tag(tag_id):
    ...
    try:
        ...
        logger.info(f"删除标签成功: {tag.name} (ID: {tag_id})")
