...
def refresh_token():
    ...
    try:
        ...
        if not payload or payload.get('type') != 'refresh':
            logger.warning("刷新token失败 - 无效的refresh_token")
            return jsonify({
                'success': False,
                'data': None,
                'message': '无效的刷新token',
        ...
        if not session:
            logger.warning("刷新token失败 - 会话不存在或已过期")
            return jsonify({
                'success': False,
                'data': None,
                'message': '会话已过期，请重新登录',
        ...
        if not user:
            logger.warning("刷新token失败 - 用户不存在")
            return jsonify({
                'success': False,
                'data': None,
                'message': '用户不存在',
        ...
        if not user.is_active:
            logger.warning("刷新token失败 - 账号已禁用")
            return jsonify({
                'success': False,
                'data': None,
                'message': '账号已被禁用',
        ...
        logger.info(f"刷新token成功: {user.username}")
    ...
    try:
        ...
        if refresh_token_str:
            ...
            if session:
                ...
                db.session.commit()
                logger.info(f"用户登出成功（会话ID: {session.id}）")
        else:
            logger.info("用户登出成功（未提供refresh_token）")
