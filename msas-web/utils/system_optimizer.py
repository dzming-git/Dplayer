...
# 创建日志记录器
optimizer_logger = logging.getLogger('system_optimizer')
...
class SystemOptimizer:
    ...
    def optimize_firewall(self, port, service_name):
        ...
        try:
            ...
            if result.returncode == 0:
                optimizer_logger.info(f'防火墙规则已添加: {rule_name}')
                return True
            else:
                optimizer_logger.warning(f'防火墙规则添加失败: {rule_name}, 错误: {result.stderr}')
                return False

        except Exception as e:
            optimizer_logger.warning(f'防火墙优化失败: {e}')
            return False
    ...
    def optimize_tcp_pool(self):
        ...
        try:
            ...
            if result.returncode != 0:
                optimizer_logger.warning(f'动态端口范围优化失败: {result.stderr}')
                success = False
            else:
                optimizer_logger.info('动态端口范围已优化: 1025-65537')
            ...
            try:
                ...
                winreg.CloseKey(key)
                optimizer_logger.info('TIME_WAIT超时时间已设置为30秒')
            except Exception as e:
                optimizer_logger.warning(f'TCP超时优化失败: {e}')
                # 尝试使用reg命令
                subprocess.run(
            ...
            optimizer_logger.warning(f'TCP连接池优化失败: {e}')
            return False
    ...
    def enable_tcp_keepalive(self):
        ...
        try:
            ...
            try:
                ...
                optimizer_logger.info('TCP保活时间已设置为60秒')
            except:
                pass  # 可能已存在

            try:
                winreg.SetValueEx(key, 'KeepAliveInterval', 0, winreg.REG_DWORD, 0x2710)
                optimizer_logger.info('TCP保活间隔已设置为10秒')
            except:
                pass  # 可能已存在
            ...
            optimizer_logger.warning(f'TCP保活机制启用失败: {e}')
            return False
    ...
    def flush_dns_cache(self):
        ...
        try:
            ...
            optimizer_logger.info('DNS缓存已刷新')
            return True
        except Exception as e:
            optimizer_logger.warning(f'DNS缓存刷新失败: {e}')
            return False
    ...
    def optimize_socket(self, sock):
        ...
        try:
            ...
            optimizer_logger.debug('Socket配置已优化')
        except Exception as e:
            optimizer_logger.warning(f'Socket优化失败: {e}')
    ...
    def optimize_all(self, ports=None, service_names=None):
        ...
        Returns:
            dict: 优化结果统计
        """
        optimizer_logger.info('=' * 50)
        optimizer_logger.info('开始执行系统网络优化...')
        optimizer_logger.info('=' * 50)
        ...
        if self.windows and not self.check_admin_rights():
            optimizer_logger.warning('警告: 未以管理员权限运行，部分优化可能失败')
            optimizer_logger.warning('建议以管理员权限启动服务')
