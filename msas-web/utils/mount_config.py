...
class MountConfig:
    ...
    def load(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                logger.info(f"加载挂载配置成功: {self.config_path}")
            else:
                # 使用默认配置
                self.mounts = self.get_default_mounts()
                logger.info(f"挂载配置文件不存在，使用默认配置")
    ...
    def add_mount(self, name: str, source: str, target: str, 
        ...
        logger.info(f"添加挂载点: {name} -> {target}")
    ...
    def save(self, path: Optional[str] = None):
        ...
        try:
            ...
            logger.info(f"保存挂载配置成功: {save_path}")
        except Exception as e:
