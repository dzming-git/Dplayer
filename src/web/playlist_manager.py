...
class PlaylistManager:
    ...
    def create_playlist(
        ...
        try:
            ...
            log.runtime('INFO', f"创建播放列表成功: {name}")
    ...
    def get_user_playlists(
        ...
        try:
            ...
            log.debug('ERROR', f"获取用户播放列表失败: {e}", exc_info=True)
            raise
    ...
    def update_playlist(
        ...
        try:
            ...
            log.runtime('INFO', f"更新播放列表成功: {playlist_id}")
            return playlist.to_dict()
    ...
    def delete_playlist(playlist_id: int) -> bool:
        ...
        try:
            ...
            log.runtime('INFO', f"删除播放列表成功: {playlist_id}")
            return True
    ...
    def add_video_to_playlist(
        ...
        try:
            ...
            if existing_item:
                log.debug('WARN', f"视频已存在于播放列表中: {video_id}")
            ...
            log.runtime('INFO', f"添加视频到播放列表成功: playlist_id={playlist_id}, video_id={video_id}")
            return item.to_dict()
    ...
    def remove_video_from_playlist(playlist_id: int, video_id: int) -> bool:
        ...
        try:
            ...
            log.runtime('INFO', f"从播放列表移除视频成功: playlist_id={playlist_id}, video_id={video_id}")
            return True
    ...
    def get_playlist_items(
        ...
        try:
            ...
            log.debug('ERROR', f"获取播放列表项失败: {e}", exc_info=True)
            raise
    ...
    def update_video_position(playlist_id: int, video_id: int, new_position: int) -> bool:
        ...
        try:
            ...
            log.runtime('INFO', f"更新视频位置成功: playlist_id={playlist_id}, video_id={video_id}, position={new_position}")
            return True

        except Exception as e:
            db.session.rollback()
            log.debug('ERROR', f"更新视频位置失败: {e}", exc_info=True)
            raise
        ...
        try:
            ...
            for video_id in video_ids:
                try:
                    ...
                    log.debug('ERROR', f"添加视频到播放列表失败: video_id={video_id}, 错误={e}")
                    results.append({
                        'video_id': video_id,
                        'success': False,
                        'error': str(e)
            ...
            log.debug('ERROR', f"批量添加视频到播放列表失败: {e}", exc_info=True)
            raise
    ...
    def set_current_video(playlist_id: int, video_id: int) -> bool:
        ...
        try:
            ...
            log.runtime('INFO', f"设置当前播放视频: playlist_id={playlist_id}, video_id={video_id}")
            return True

        except Exception as e:
            db.session.rollback()
            log.debug('ERROR', f"设置当前播放视频失败: {e}", exc_info=True)
            raise
