...
class ResourceMonitor:
    ...
    def start_monitoring(self):
        """开始监控"""
        if self.monitoring:
            log.debug('WARN', "资源监控已经在运行")
            return

        self.monitoring = True
        self.monitor_thread = threading.Thread(target=self._monitor_loop, daemon=True)
        self.monitor_thread.start()
        log.runtime('INFO', "资源监控已启动")
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            try:
                ...
                log.debug('ERROR', f"资源检查失败: {e}", exc_info=True)
    ...
    def _handle_violation(self, resource_type: ResourceType, current: float, limit: float):
        ...
        for callback in self.callbacks:
            try:
                ...
                log.debug('ERROR', f"回调函数执行失败: {e}", exc_info=True)
...
class ResourceLimiter:
    ...
    def _pause_processing(self, reason: str):
        ...
        if not self.paused:
            ...
            log.debug('WARN', f"因{reason}暂停处理")
            # 30秒后自动恢复
    ...
    def _resume_processing(self):
        ...
        if self.paused:
            ...
            log.runtime('INFO', "处理已恢复")

    def wait_if_paused(self):
        """如果暂停则等待"""
        while self.paused:
            log.runtime('INFO', "等待资源释放...")
