...
def is_port_in_use(port):
    ...
    try:
        ...
        log.debug('ERROR', f"检查端口 {port} 占用状态失败: {e}")
        return False
        ...
        for conn in psutil.net_connections():
            if conn.laddr.port == port and conn.status == 'LISTEN':
                try:
                    ...
                    log.debug('WARN', f"无法访问进程 {conn.pid}")
    except Exception as e:
        log.debug('ERROR', f"查找端口 {port} 的进程失败: {e}")
...
def kill_process(process):
    ...
    try:
        ...
        try:
            ...
            log.runtime('INFO', f"成功终止进程 {name} (PID: {pid})")
            return True
        except psutil.TimeoutExpired:
            # 正常终止超时，强制杀死
            process.kill()
            process.wait(timeout=2)
            log.runtime('INFO', f"强制杀死进程 {name} (PID: {pid})")
            return True
            
    except psutil.NoSuchProcess:
        log.debug('WARN', f"进程已不存在")
        return True
    except psutil.AccessDenied:
        log.debug('ERROR', f"没有权限终止进程 (PID: {process.pid})")
        return False
    except Exception as e:
        log.debug('ERROR', f"终止进程失败: {e}")
        return False
...
def kill_all_processes_using_port(port):
    ...
    if not processes:
        log.runtime('INFO', f"端口 {port} 没有被占用")
        return result
    
    log.debug('WARN', f"端口 {port} 被 {len(processes)} 个进程占用")
    ...
    if is_port_in_use(port):
        ...
        log.debug('ERROR', f"端口 {port} 仍然被占用，可能有新进程启动")
    else:
        log.runtime('INFO', f"端口 {port} 已成功释放")
...
def ensure_port_available(port):
    ...
    log.debug('WARN', f"端口 {port} 被占用，正在强制释放...")
