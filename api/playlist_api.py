...
def create_playlist():
    ...
    try:
        ...
        logger.info(f"创建播放列表: {name}, 视频数: {len(video_ids)}")
...
def update_playlist(playlist_id):
    ...
    try:
        ...
        logger.info(f"更新播放列表: {playlist.name}")
...
def delete_playlist(playlist_id):
    ...
    try:
        ...
        logger.info(f"删除播放列表: {playlist.name}")
...
def add_to_playlist(playlist_id):
    ...
    try:
        ...
        logger.info(f"添加{added_count}个视频到播放列表: {playlist.name}")
...
def remove_from_playlist(playlist_id, item_id):
    ...
    try:
        ...
        logger.info(f"从播放列表移除视频: {playlist.name}")
...
def reorder_playlist(playlist_id):
    ...
    try:
        ...
        logger.info(f"重新排序播放列表: {playlist.name}")
        ...
        db.session.rollback()
        logger.error(f"重新排序播放列表失败: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def play_playlist(playlist_id):
    ...
    try:
        ...
        logger.info(f"播放播放列表: {playlist.name}")
        ...
        db.session.rollback()
        logger.error(f"播放播放列表失败: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def shuffle_playlist(playlist_id):
    ...
    try:
        ...
        logger.info(f"随机排序播放列表: {playlist.name}")
        ...
        db.session.rollback()
        logger.error(f"随机播放列表失败: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
