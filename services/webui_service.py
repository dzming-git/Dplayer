...
check_service_launch('DPlayer WebUI Service', 'services/webui_service.py')
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, 'webui_service.log'), encoding='utf-8'),
...
def check_node_modules():
    ...
    if not os.path.exists(node_modules):
        logger.error(f"node_modules 不存在，请先运行: cd {FRONTEND_DIR} && npm install")
        return False
    return True
...
def start_webui():
    """启动 WebUI 服务"""
    logger.info("=" * 60)
    logger.info("DPlayer WebUI 服务启动")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(f"WebUI 服务已启动，PID: {process.pid}")
        ...
        logger.info(f"WebUI 服务已退出，返回码: {return_code}")
